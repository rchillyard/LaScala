...
/**
  ...
  */
case class RenderableFunction[R: ClassTag](arity: Int, w: FunctionString, cbn: List[Boolean], cs: ParamClasses)(f: (Product) => R) extends (Product => Try[R]) with OldRenderable with Serializable {
...
object RenderableFunction {
  ...
  private def invert[T, R: ClassTag](n: Int, arity: Int, f: (Product) => R, w: FunctionString, cbn: List[Boolean], cs: ParamClasses): RenderableFunction[R] = {
    if (arity < 2 || n > arity)
