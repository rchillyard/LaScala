...
/**
  ...
  */
case class OldRenderableFunction[R: ClassTag](arity: Int, w: FunctionString, cbn: List[Boolean], cs: ParamClasses)(f: (Product) => R) extends (Product => Try[R]) with OldRenderable with Serializable {
  ...
  def apply(p: Product): Try[R] =
    if (arity == p.productArity) {
      ...
      val ry = OldRenderableFunction.unwrapIfNecessary(Try(g(p)), returnIsTry, s"apply($p): logic error")
      for (r <- ry) yield OldRenderableFunction.toScala[R](r)
    }
    else
      Failure(OldRenderableFunctionException(s"cannot apply Product of arity ${p.productArity}: $p to RenderableFunction with arity $arity: ($this)"))
  ...
  else throw OldRenderableFunctionException(s"apply($t): arity is not one ($arity) for $this")
  ...
  else throw OldRenderableFunctionException(s"callByName(): arity is not zero ($arity) for $this")
  ...
  /**
    ...
    */
  def partiallyApplyParameters(tps: Seq[Parameter[_]]): Try[OldRenderableFunction[R]] = {
    def partiallyApplyParameter(rfy: Try[OldRenderableFunction[R]], tp: Parameter[_], x: ClassTag[_]): Try[OldRenderableFunction[R]] = tp match {
      case Left(t) => // t: T
    ...
    @tailrec
    def inner(rfy: Try[OldRenderableFunction[R]], work: Seq[(Parameter[_], ClassTag[_])]): Try[OldRenderableFunction[R]] =
      work match {
        case Nil => rfy
  ...
  def asFunction: () => R = if (arity == 0)
    () => {
      ...
      f match {
        case r: R => r
        case g: Function[Unit, _]@unchecked => OldRenderableFunction.toScala[R](g(()))
        // NOTE: this could result in the throwing of an exception
        case x => OldRenderableFunction.toScala[R](x)
      }
    }
  else throw OldRenderableFunctionException(s"asFunction: arity is not zero ($arity) for $this")
  ...
  /**
    ...
    */
  def partiallyApply[T: ClassTag](t: Any): Try[OldRenderableFunction[R]] =
    OldRenderableFunction.partiallyApply(arity, func, t.asInstanceOf[T], w, cbn, cs)
  ...
  /**
    ...
    * @return a RenderableFunction[R] wrapped in Try
    */
  def partiallyApplyFunction[T](f: () => T, z: FunctionString): Try[OldRenderableFunction[R]] =
    OldRenderableFunction.partiallyApplyFunction(arity, func, f, w.partiallyApplyFunction(z), cbn, cs)
  ...
  /**
    ...
    */
  def invert[T](n: Int): OldRenderableFunction[R] = OldRenderableFunction.invert(n, arity, func, w, cbn, cs)
...
case class FunctionString(f: String, ps: List[Param]) {
  ...
  def partiallyApply[T](t: => T, skip: Int = 0): FunctionString =
    ...
    else throw OldRenderableFunctionException(s"FunctionString.partiallyApply cannot apply $t at $skip to $this")
  ...
  def partiallyApplyFunction(w: FunctionString, skip: Int = 0): FunctionString =
    ...
    else throw OldRenderableFunctionException(s"FunctionString.partiallyApply cannot apply $w at $skip to $this where arity <= $skip")
...
object OldRenderableFunction {
  ...
  def convert[T: ClassTag](x: Any): T = x match {
    ...
    case _ =>
      ...
      else throw OldRenderableFunctionException(s"$x of type ${x.getClass} cannot be converted to $tc")
  }

  // CONSIDER using use assert instead
  def asserting[A](b: => Boolean, w: => String, f: => A): A = if (b) f else throw AssertingError(w)

  def apply[R: ClassTag](arity: Int, func: (Product) => R, w: String, cbn: Seq[Boolean], cs: ParamClasses): OldRenderableFunction[R] = OldRenderableFunction(arity, FunctionString(w, arity, cbn), cbn.toList, cs)(func)
  ...
  /**
    ...
    */
  def apply[R: ClassTag](f: () => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = {
    asserting(f != null && w != null, "f or w is null",
      apply(0, w, cbn.toList, Nil)(asTupledFunctionType(f))
    )
  }

  def apply[R: ClassTag](f: () => R, w: String, cbn: Seq[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 1, cbn), cbn)

  def apply[T: ClassTag, R: ClassTag](f: T => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = {
    asserting(f != null && w != null, "f or w is null",
      apply(1, w, cbn.toList, List(implicitly[ClassTag[T]]))(asTupledFunctionType(f))
    )
  }

  def apply[T: ClassTag, R: ClassTag](f: T => R, w: String, cbn: Seq[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 1, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, R: ClassTag](f: (T1, T2) => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
    apply(2, w, cbn.toList, List(implicitly[ClassTag[T1]], implicitly[ClassTag[T2]]))(asTupledFunctionType(f))
  )

  def apply[T1: ClassTag, T2: ClassTag, R: ClassTag](f: (T1, T2) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 2, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, R: ClassTag](f: (T1, T2, T3) => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
    apply(3, w, cbn.toList, List(implicitly[ClassTag[T1]], implicitly[ClassTag[T2]], implicitly[ClassTag[T3]]))(asTupledFunctionType(f))
  )

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, R: ClassTag](f: (T1, T2, T3) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 3, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, R: ClassTag](f: (T1, T2, T3, T4) => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
    apply(4, w, cbn.toList, List(implicitly[ClassTag[T1]], implicitly[ClassTag[T2]], implicitly[ClassTag[T3]], implicitly[ClassTag[T4]]))(asTupledFunctionType(f))
  )

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, R: ClassTag](f: (T1, T2, T3, T4) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 4, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5) => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
    apply(5, w, cbn.toList, List(implicitly[ClassTag[T1]], implicitly[ClassTag[T2]], implicitly[ClassTag[T3]], implicitly[ClassTag[T4]], implicitly[ClassTag[T5]]))(asTupledFunctionType(f))
  )

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 5, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6) => R, w: FunctionString, cbn: Seq[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
    apply(6, w, cbn.toList, List(implicitly[ClassTag[T1]], implicitly[ClassTag[T2]], implicitly[ClassTag[T3]], implicitly[ClassTag[T4]], implicitly[ClassTag[T5]], implicitly[ClassTag[T6]]))(asTupledFunctionType(f))
  )

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 6, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, T7: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6, T7) => R, w: FunctionString, cbn: List[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
  ...
  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, T7: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6, T7) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 7, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, T7: ClassTag, T8: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6, T7, T8) => R, w: FunctionString, cbn: List[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
  ...
  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, T7: ClassTag, T8: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6, T7, T8) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 8, cbn), cbn)

  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, T7: ClassTag, T8: ClassTag, T9: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6, T7, T8, T9) => R, w: FunctionString, cbn: List[Boolean]): OldRenderableFunction[R] = asserting(f != null && w != null, "f or w is null",
  ...
  def apply[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag, T5: ClassTag, T6: ClassTag, T7: ClassTag, T8: ClassTag, T9: ClassTag, R: ClassTag](f: (T1, T2, T3, T4, T5, T6, T7, T8, T9) => R, w: String, cbn: List[Boolean]): OldRenderableFunction[R] = apply(f, FunctionString(w, 9, cbn), cbn)
  ...
  /**
    ...
    * @return a RenderableFunction which yields a List[T]
    */
  def varargs[T: ClassTag](n: Int): OldRenderableFunction[Seq[T]] = {
    val mkList = "mkList"
    val cbn = callByValue(n)
    val cs: ParamClasses = (Stream.continually(implicitly[ClassTag[T]].asInstanceOf[ClassTag[Any]]) take n).toList
  ...
  def tupled[T1: ClassTag, R: ClassTag](f: T1 => R): Tuple1[T1] => R = {
    case Tuple1(x1) =>
      ...
      else throw OldRenderableFunctionException(s"$f is not a Function1[${implicitly[ClassTag[T1]]},${implicitly[ClassTag[R]]}]")
  }
  ...
  private def unwrap[R](ry: Try[R], msg: String): Try[R] = ry match {
    ...
    case _ => Failure(OldRenderableFunctionException(msg))
  }
  ...
  /**
    ...
    * @throws OldRenderableFunctionException if unable to properly for a T value from x
    */
  private def extract[T: ClassTag](x: Any): T = x match {
    ...
    case t: T => t
    case t =>
      try OldRenderableFunction.toScala[T](t)
      catch {
        case z: Exception => throw OldRenderableFunctionException(s"cannot extract a ${implicitly[ClassTag[T]]} value from $x", z)
      }
  }
  ...
  /**
    ...
    */
  /** private */
  def partiallyApply[T, R: ClassTag](n: Int, f: (Product) => R, t: T, w: FunctionString, cbn: Seq[Boolean], cs: ParamClasses): Try[OldRenderableFunction[R]] = Try {
    val g = getPartialApplicationFunction(n, f, t, cbn.head)(cs.head.asInstanceOf[ClassTag[T]], implicitly[ClassTag[R]])
    OldRenderableFunction(n - 1, w.partiallyApply(t), cbn.tail.toList, cs.tail)(g)
  }
  ...
  /**
    ...
    */
  private def partiallyApplyFunction[T, R: ClassTag](n: Int, f: (Product) => R, tf: () => T, w: FunctionString, cbn: Seq[Boolean], cs: ParamClasses): Try[OldRenderableFunction[R]] = Try {
    val g = if (cbn.head)
    ...
    OldRenderableFunction(n - 1, w, cbn.tail.toList, cs.tail)(g)
  }
  ...
  if (cbn) getPartialApplicationFunction(n, f, { () => t }, false)
    ...
    n match {
      ...
      case _ =>
        throw OldRenderableFunctionException(s"cannot partially apply function of arity $n (not supported/implemented)")
    }
  ...
  private def getPartialApplicationFunctionCallByName[T: ClassTag, R: ClassTag](n: Int, f: (Product) => R, t: () => T): (Product) => R =
    n match {
      ...
      case 7 =>
        ...
        g6
      case _ => throw OldRenderableFunctionException(s"FunctionCalls with arity $n are not supported")
    }

  private def invert[X](n: Int, xs: List[X]): List[X] = ((xs take (xs.size - n)) :+ xs(n - 1) :+ xs(n - 2)) ++ (xs drop n)

  private def invert[T, R: ClassTag](n: Int, arity: Int, f: (Product) => R, w: FunctionString, cbn: List[Boolean], cs: ParamClasses): OldRenderableFunction[R] = {
    if (arity < 2 || n > arity)
      throw OldRenderableFunctionException(s"can't invert $n parameters of a RenderableFunction of arity $arity")
    else {
      ...
      arity match {
        case 2 =>
          ...
          val f2i = n match {
            ...
            case _ => throw OldRenderableFunctionException(s"cannot invert $n parameters when arity = $arity")
          }
          val g2 = asFunctionType { x: (T, T) => f2i(x._1)(x._2) }
          OldRenderableFunction(n, w.invert(n), cbni, csi)(g2)
        case 3 =>
          ...
          OldRenderableFunction(n, w.invert(n), cbni, csi)(g3)
        case 4 =>
          ...
          OldRenderableFunction(n, w.invert(n), cbni, csi)(g4)
        case 5 =>
          ...
          OldRenderableFunction(n, w.invert(n), cbni, csi)(g5)
        case 6 =>
          ...
          OldRenderableFunction(n, w.invert(n), cbni, csi)(g6)
        case _ => throw OldRenderableFunctionException(s"invert with arity $arity is not supported")
      }
    }
  }
}
...
/**
  ...
  * @param s the message
  */
case class OldRenderableFunctionException(s: String, t: Throwable = null) extends Exception(s, t)
