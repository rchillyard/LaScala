...
/**
  ...
  */
case class Leaf[+A](a: A) extends Node[A] {
...
object TreeLike {
  /**
    ...
    * @return a Parent of type Node[A]
    */
  implicit def treeLikeParent[A]: Parent[TreeLike[A]] = new Parent[TreeLike[A]] {
    /**
      * Get the children of this Node, if any.
      *
      * @return the children as a Seq of Nodes
      */
    def children(t: TreeLike[A]): Seq[TreeLike[A]] = t.children map {
      _ match {
        case x: TreeLike[A] => x
        case x => throw TreeException(s"logic error: $x doesn't implement TreeLike")
      }
    }
  }
}
