...
/**
  ...
  */
case class OldRenderableTraversable(xs: Traversable[_], bookends: String = "(,)", linear: Boolean = false, max: Option[Int] = None) extends OldRenderable {
  def render(indent: Int)(implicit tab: (Int) => Prefix): String = {
    ...
    OldRenderable.elemsToString(xs, indent, p, q, r, max)
  }
}
...
/**
  ...
  */
case class OldRenderableProduct(p: Product) extends OldRenderable {
  def render(indent: Int)(implicit tab: (Int) => Prefix): String = OldRenderable.elemsToString(p.productIterator.toSeq, indent, s"${p.productPrefix}(", ",", ")")
}
...
/**
  ...
  */
case class OldRenderableCaseClass[T: TypeTag](t: T) extends OldRenderable {
  ...
  def render(indent: Int = 0)(implicit tab: (Int) => Prefix): String = {
    ...
    for (((k, _), v) <- parameters) sb.append(nl(indent + 1) + s"$k:" + OldRenderable.renderElem(v, indent + 1))
    sb.append(nl(indent + 1) + ")")
    sb.toString()
  }
}

object OldRenderableCaseClass {
...
/**
  ...
  */
case class OldRenderableOption(xo: Option[_]) extends OldRenderable {
  def render(indent: Int)(implicit tab: (Int) => Prefix): String = xo match {
    case Some(x) => s"Some(" + OldRenderable.renderElem(x, indent) + ")"
    case _ => "None"
  }
}
...
/**
  ...
  */
case class OldRenderableTry(xy: Try[_]) extends OldRenderable {

  def render(indent: Int)(implicit tab: (Int) => Prefix): String = xy match {
    case Success(x) => s"Success(" + OldRenderable.renderElem(x, indent) + ")"
...
/**
  ...
  */
case class OldRenderableEither(e: Either[_, _]) extends OldRenderable {

  def render(indent: Int)(implicit tab: (Int) => Prefix): String = e match {
    case Left(x) => s"Left(" + OldRenderable.renderElem(x, indent) + ")"
    case Right(x) => s"Right(" + OldRenderable.renderElem(x, indent) + ")"
  }
}

object OldRenderable {
  ...
  /**
    ...
    * @return a Renderable
    */
  implicit def renderableTraversable(xs: Traversable[_]): OldRenderable = OldRenderableTraversable(xs, max = Some(MAX_ELEMENTS))

  implicit def renderableOption(xo: Option[_]): OldRenderable = OldRenderableOption(xo)

  implicit def renderableTry(xy: Try[_]): OldRenderable = OldRenderableTry(xy)

  implicit def renderableEither(e: Either[_, _]): OldRenderable = OldRenderableEither(e)

  implicit def renderableProduct(p: Product): OldRenderable = OldRenderableProduct(p)
  ...
  def renderElem(elem: Any, indent: Int)(implicit tab: (Int) => Prefix): String = elem match {
    ...
    case s: String => s""""$s""""
    case r: OldRenderable => r.render(indent + 1)
    case x => x.toString
  }
