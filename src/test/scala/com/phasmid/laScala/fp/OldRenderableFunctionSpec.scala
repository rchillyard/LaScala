...
import com.phasmid.laScala.fp.OldRenderableFunction.getPartialApplicationFunction
...
/**
  ...
  */
class OldRenderableFunctionSpec extends FlatSpec with Matchers with PrivateMethodTester {
  ...
  it should "yield true with arity 0" in {
    ...
    val f = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(0))
    val r = f(Tuple0)
  ...
  it should "yield Hello with arity 1" in {
    ...
    val f = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(1))
  ...
  it should "yield Hello with arity 2" in {
    ...
    val f = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(2))
  ...
  it should "yield Hello with arity 3" in {
    ...
    val f = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(3))
  ...
  it should "handle function returning a Try" in {
    ...
    // CONSIDER this should use a call-by-name parameter
    val f = OldRenderableFunction(lookup _, "lookup", OldRenderableFunction.callByName(1))
    val r = f(Tuple1("x"))
  ...
  it should "apply a 2-arity function" in {
    ...
    val f = OldRenderableFunction(render _, name, OldRenderableFunction.callByValue(2))
    f.w shouldBe FunctionString(name, arity)
    f.arity shouldBe arity
  ...
  it should "work with partiallyApply (1)" in {
    ...
    val f = OldRenderableFunction(render _, name, OldRenderableFunction.callByValue(2))
    val g1y: Try[OldRenderableFunction[String]] = f.partiallyApply("Hello")
    for (g1 <- g1y) yield g1.arity should matchPattern { case Success(1) => }
    val g2y: Try[OldRenderableFunction[String]] = for (g <- g1y; h <- g.partiallyApply("Goodbye")) yield h
  ...
  it should "work with partiallyApply (2)" in {
    ...
    val f = OldRenderableFunction(and _, name, List(false, true))
    val bt: Boolean = true
    val bf: Boolean = false
    val g1y: Try[OldRenderableFunction[Boolean]] = f.partiallyApply(bt)
    val g2y: Try[OldRenderableFunction[Boolean]] = for (g <- g1y; h <- g.partiallyApply(bf)) yield h
  ...
  it should "work with partiallyApply (3)" in {
    ...
    val f = OldRenderableFunction(and _, name, List(false, true))
    val g1y: Try[OldRenderableFunction[Boolean]] = f.partiallyApply("true")
    val g2y: Try[OldRenderableFunction[Boolean]] = for (g <- g1y; h <- g.partiallyApply("false")) yield h
    val by: Try[Boolean] = for (g2 <- g2y; b <- g2.callByName()) yield b
    by should matchPattern { case Success(false) => }
  }
  ...
  it should "work with partiallyApply (4)" in {
    ...
    val f = OldRenderableFunction(and _, name, List(false, true))
    val g1y: Try[OldRenderableFunction[String]] = f.partiallyApply("true")
    val g2y: Try[OldRenderableFunction[String]] = for (g <- g1y; h <- g.partiallyApply("false")) yield h
  ...
  it should "partiallyApply correctly with index 0" in {
    ...
    an[OldRenderableFunctionException] should be thrownBy FunctionString("f", 0).partiallyApply("x")
  }

  it should "partiallyApply correctly with index 1" in {
    FunctionString("f", 2).partiallyApply("x", 1).toString shouldBe """f(a?)("x")"""
    an[OldRenderableFunctionException] should be thrownBy FunctionString("f", 0).partiallyApply("x", 1)
    an[OldRenderableFunctionException] should be thrownBy FunctionString("f", 1).partiallyApply("x", 2)
  }
  ...
  it should "reduce arity 4 to 3" in {
    ...
    val f = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(4))
    val g = f.partiallyApply(Some("Hello"))
    g should matchPattern { case Success(_) => }
  ...
  it should "handle a function (1)" in {
    ...
    val lookup: OldRenderableFunction[String] = OldRenderableFunction(fLookup _, "lookup", Seq(true))

    def show(s1: String) = s1

    val f = OldRenderableFunction(show _, "render", OldRenderableFunction.callByValue(1))
  ...
  it should "handle a function that throws an exception (1)" in {
    ...
    val f = OldRenderableFunction(show _, "render", OldRenderableFunction.callByValue(1))
    val fy: Try[OldRenderableFunction[String]] = f.partiallyApplyParameters(Seq(Right(Closure(lookup, Left("pi")))).asInstanceOf[Seq[Parameter[_]]])
    val ry = for (g: OldRenderableFunction[String] <- fy; r <- g.callByName()) yield r
  ...
  ignore should "handle a function that throws an exception 2)" in {
    ...
    cy should matchPattern { case Failure(_: OldRenderableFunctionException) => }
  }

  it should "deal with not" in {
    def fNot(p1: Boolean): Boolean = {println(s"call NOT $p1"); !p1}

    val f = OldRenderableFunction(fNot _, "not", OldRenderableFunction.callByValue(1))
  ...
  it should "deal with NOT IN" in {
    val f_1: OldRenderableFunction[Boolean] = OldRenderableFunction({ b: Boolean => println(s"call NOT $b"); !b }, "NOT", OldRenderableFunction.callByValue(1))
    f_1.arity shouldBe 1
    val g_2: OldRenderableFunction[Boolean] = OldRenderableFunction((p0: String, p1: List[String]) => {
      println(s"$p0 IN $p1")
      p1 contains p0
    }, "IN", OldRenderableFunction.callByValue(2))
  ...
  it should "yield the same value for two simple functions" in {
    ...
    val f1 = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(1))
    val f2 = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(1))
  ...
  it should "yield the same value when partially applied" in {
    ...
    val f1 = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(1))
    val f2 = OldRenderableFunction(render _, "render", OldRenderableFunction.callByValue(1))
    val rf1y: Try[OldRenderableFunction[String]] = f1.partiallyApply("k")
    rf1y should matchPattern { case Success(_) => }
    val rf2y: Try[OldRenderableFunction[String]] = f2.partiallyApply("k")
  ...
  it should "handle varargs of cardinality 0" in {
    val rf1: OldRenderableFunction[Seq[String]] = OldRenderableFunction.varargs(0)
    rf1.arity shouldBe 0
    val rf2y = rf1.partiallyApplyParameters(List[Parameter[_]]())
    rf2y should matchPattern { case Success(_) => }
    rf2y.get.callByName shouldBe Success(List())
  }

  it should "handle varargs of cardinality 1" in {
    val rf1: OldRenderableFunction[Seq[Seq[String]]] = OldRenderableFunction.varargs(1)
    rf1.arity shouldBe 1
    val rf2y = rf1.partiallyApplyParameters(List[Parameter[_]](Left("l")))
    rf2y should matchPattern { case Success(_) => }
    rf2y.get.callByName shouldBe Success(List("l"))
  }

  it should "handle varargs of cardinality 2" in {
    val rf1: OldRenderableFunction[Seq[String]] = OldRenderableFunction.varargs[String](2)
  ...
  it should "handle varargs of cardinality " + args.toString in {
    val rf1: OldRenderableFunction[Seq[String]] = OldRenderableFunction.varargs[String](args)
  ...
  it should """evaluate to true with true & true""" in {
    ...
    val ifAnd = OldRenderableFunction(fAnd _, "and", List(false, true))
  ...
  it should """evaluate to java.lang.Boolean(true) with true & not (java) true""" in {
    ...
    val ifNot = OldRenderableFunction(fNot _, "not", OldRenderableFunction.callByValue(1))
  ...
  it should "convert Integer" in {
    ...
    val y = OldRenderableFunction.toScala[Int](x)
    y shouldEqual 42
  ...
  it should "convert Boolean" in {
    ...
    val y = OldRenderableFunction.toScala[Boolean](x)
    y shouldEqual true
  ...
  it should "convert Double" in {
    ...
    val y = OldRenderableFunction.toScala[Double](x)
  ...
  private def testPartiallyApply1[T1: ClassTag, R: ClassTag](expected: R, t1: T1, f: Product => R, w: String, cbn: Boolean*) = {
    ...
    //    println(h)
    val g2y = OldRenderableFunction.partiallyApply[T1, R](arity, f, t1, functionString, cbn, classTags)
