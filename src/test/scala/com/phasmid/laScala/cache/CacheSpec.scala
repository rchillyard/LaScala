...
class CacheSpec extends FlatSpec with Matchers with Futures with ScalaFutures {
  ...
  class MockLoggingAdapter extends LoggingAdapter {
    ...
    override protected def notifyInfo(message: String): Unit = {println("info: "+message); infoCount=infoCount+1}
    ...
    override protected def notifyError(message: String): Unit = {System.err.println("error: "+message); errorCount=errorCount+1}

    override protected def notifyError(cause: Throwable, message: String): Unit = {System.err.println("error: "+message+": "+cause); errorCount=errorCount+1}

    override def isWarningEnabled: Boolean = true

    override protected def notifyWarning(message: String): Unit = {System.err.println("warning: "+message); warningCount=warningCount+1}
  ...
  "NonExpiringCache" should "succeed" in {
    ...
    mockLoggingAdapter.warningCount shouldBe 0
    implicit def carper(s: String): Unit = mockLoggingAdapter.warning(s)
    def evaluate(k: String): Try[Int] = {mockLoggingAdapter.debug(s"evaluating $k"); Try(k.toInt)}
